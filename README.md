...
- **Saffron tool window** (right side), seven tabs. **Files**: feature files with scenario counts, search, tick and Run Selected, Run All, Open Report, Accept All Proposals, last-run totals, pending proposals. **Last Run**: what failed or healed in the last run, with the failed step; double-click opens the screenshot of the page at that moment. **Proposals**: tick, read the narrative, the proof-replay verdict and the action-level diff of what the proposal changes, then Accept Selected / Reject Selected / Accept All. **Tags**: tick tags and Run Tagged. **Health**: vocabulary counts, divergent steps, near-duplicate wordings, effective config. **Orphans**: caches and pending proposals whose scenario no longer exists, with the reason; double-click to open one, Remove All to run `saffron prune --yes` after a confirmation. **Dashboard**: the run report embedded in the IDE. The tabs (plugin 0.2.1 and later) read `saffron status --json` from saffron-ai 0.5.4 or later.
