...
- **Saffron tool window** (right side), five tabs. **Files**: feature files with scenario counts, search, tick and Run Selected, Run All, Open Report, Accept All Proposals, last-run totals, pending proposals. **Proposals**: tick, read the narrative and proof-replay verdict, Accept Selected / Reject Selected / Accept All. **Tags**: tick tags and Run Tagged. **Health**: vocabulary counts, divergent steps, near-duplicate wordings, effective config. **Dashboard**: the run report embedded in the IDE. The tabs read `saffron status --json` from saffron-ai 0.5.4 or later.
